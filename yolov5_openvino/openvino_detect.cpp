...
std::vector<yolo_detct::Object> yolo_detct::work(Mat img)
{
    ...
    for (int i = 0; i < output.size(); i++)
    {
        ...
        rectangle(img, Point(box.x, box.y), Point(xmax, ymax), Scalar(0, 255, 0), 3);
        rectangle(img, Point(box.x, box.y - 20), Point(xmax, box.y), Scalar(0, 255, 0), FILLED);
        std::string confidence_str = fmt::format("{:.2f}", detection.prob);
        putText(img, confidence_str, Point(box.x + 40, box.y), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(0, 0, 0));
        putText(img, class_names[detection.label], Point(box.x, box.y), FONT_HERSHEY_SIMPLEX, 0.7,
                Scalar(0, 0, 0));
    }
    imshow("detect", img);
